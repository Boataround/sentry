...
# Ensure the container connects to the RDS database
echo "Connecting to the external PostgreSQL database..."
...
if [ $RETRIES -eq 0 ]; then
  echo "Failed to connect to PostgreSQL database at $SENTRY_DB_HOST"
  exit 1
fi

echo "Connected to the PostgreSQL database at $SENTRY_DB_HOST"
